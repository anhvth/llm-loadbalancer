...
class LoadBalancerApp:
    ...
    def _summarize_health(self, results: list[EndpointCheckResult]) -> None:
        ...
        for label, state in endpoints.items():
            models = state.get("models") or []
            model_key = os.path.basename(models[0]) if models else "unknown"
            groups.setdefault(model_key, []).append((label, state))

        rows = []
        for model_key, entries in sorted(groups.items()):
            up_count = sum(1 for _, s in entries if s["status"] == "up")
            total = len(entries)
            total_reqs = sum(s.get("requests", 0) for _, s in entries)
            err_count = sum(
                1 for _, s in entries
                if s.get("error") and s["error"] != "-"
            )
            sorted_labels = sorted(entries, key=lambda x: x[0])
            endpoint_str = ",".join(label for label, _ in sorted_labels)
            rows.append((model_key, up_count, total, total_reqs, err_count, endpoint_str))

        headers = ["MODEL", "UP/TOTAL", "REQS", "ERR", "ENDPOINTS"]
        col_widths = [max(len(str(row[i])) for row in rows + [headers]) for i in range(5)]
        lines = []
        header_line = "  ".join(h.ljust(col_widths[i]) for i, h in enumerate(headers))
        lines.append(header_line)
        for row in rows:
            model_key, up_count, total, total_reqs, err_count, endpoint_str = row
            line = "  ".join([
                str(model_key).ljust(col_widths[0]),
                f"{up_count}/{total}".ljust(col_widths[1]),
                str(total_reqs).ljust(col_widths[2]),
                str(err_count).ljust(col_widths[3]),
                endpoint_str.ljust(col_widths[4]),
            ])
            lines.append(line)
        logger.info("\n".join(lines))
